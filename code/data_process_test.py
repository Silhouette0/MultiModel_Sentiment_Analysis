...
class SentenceDataset(Dataset):
    def __init__(self, opt, data_path, text_tokenizer, photo_path, image_transforms, data_type,
                 data_translation_path=None, image_coordinate=None):
    ...
    def __getitem__(self, index):
        ...
        return self.text_to_id[index], image_origin, self.label_list[index], self.text_translation_to_id[
            self.data_id_list[index]], image_augment, self.data_id_list[index]
...
class Collate():
    ...
    def _collate(self, batch_data):
        ...
        image_origin = torch.FloatTensor(np.array([np.array(b[1]) for b in batch_data]))
        label = torch.LongTensor([b[2] for b in batch_data])
        text_translation_to_id = [torch.LongTensor(b[3]) for b in batch_data]
        image_augment = torch.FloatTensor(np.array([np.array(b[4]) for b in batch_data]))
        ...
        ids = [int(b[-1]) for b in batch_data]

        max_length = max(data_length)
        if max_length < self.min_length:
            # 这一步防止在后续的计算过程中，因为文本长度和mask长度不一致而出错
            text_to_id[0] = torch.cat(
                (text_to_id[0], torch.LongTensor([0] * (self.min_length - text_to_id[0].size(0)))))
        ...
        if max_translation_length < self.min_length:
            ...
            text_translation_to_id[0] = torch.cat((text_translation_to_id[0], torch.LongTensor(
                [0] * (self.min_length - text_translation_to_id[0].size(0)))))
        ...
        return text_to_id, torch.LongTensor(bert_attention_mask), image_origin, torch.LongTensor(
            text_image_mask), label, \
            text_translation_to_id, torch.LongTensor(tran_bert_attention_mask), image_augment, torch.LongTensor(
            tran_text_image_mask), target_labels, ids
...
def data_process(opt, data_path, text_tokenizer, photo_path, data_type, data_translation_path=None,
