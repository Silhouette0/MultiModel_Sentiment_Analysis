...
class ModelParam:
    def __init__(self, texts=None, images=None, bert_attention_mask=None, text_image_mask=None, segment_token=None,
                 image_coordinate_position_token=None):
        self.texts = texts
        self.images = images
    ...
    def set_data_param(self, texts=None, images=None, bert_attention_mask=None, text_image_mask=None,
                       segment_token=None, image_coordinate_position_token=None):
        self.texts = texts
        self.images = images
        self.bert_attention_mask = bert_attention_mask
        self.text_image_mask = text_image_mask
...
class ImageModel(nn.Module):
    def __init__(self, opt):
        ...
        print("加载convnext_base")

        self.convnext_encoder = self.convnext
        self.convnext_avgpool = nn.Sequential(
    ...
    def forward(self, x):
        _, image_encoder = self.convnext_encoder(x)
        image_cls = image_encoder
...
class FuseModel(nn.Module):
    ...
    def forward(self, text_inputs, bert_attention_mask, image_inputs, text_image_mask):
        ...
        # 消融实验
        if self.opt.my_type == 3:
            text_image_cat = image_init
        elif self.opt.my_type == 4:
            text_image_cat = text_init
        else:
...
class BertModel(nn.Module):
    def __init__(self, opt):
        ...
        self.config = BertConfig.from_pretrained(abl_path + 'bert-base-uncased/')
        self.model = BertForPreTraining.from_pretrained(abl_path + 'bert-base-uncased/', config=self.config)
        self.bert = BertForPreTraining.from_pretrained(abl_path + 'bert-base-uncased/', config=self.config)
        self.bert.cls.seq_relationship = nn.Linear(768, 3, True)

    def forward(self, data_orgin: ModelParam, data_augment: ModelParam = None, labels=None, target_labels=None):
        output = self.bert(data_orgin.texts, data_orgin.bert_attention_mask)[1]
        return output
...
class ConvNeXtModel(nn.Module):
    def __init__(self, opt):
        ...
        print("-" * 50)
        print("ConvNeXtModel")
        self.model = convnext_base(pretrained=False, num_classes=3)

    def forward(self, data_orgin: ModelParam, data_augment: ModelParam = None, labels=None, target_labels=None):
        output = self.model(data_orgin.images)
        return output[0]
